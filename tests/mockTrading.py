...
class MockTrading:
    ...
    def _createOrder(self,X_size,Y_size,X_price,Y_price):
        ...
        X_required_margin = self.margin_manager.calculate_required_margin(XSymbolInfo.SYMBOL_NAME, X_size, 1 if X_size > 0 else -1, XSymbolInfo.SYMBOL_LEVERAGE)
        self.position.append(Position(XSymbolInfo.SYMBOL_NAME,X_size,1 if X_size > 0 else -1,X_price,X_required_margin, XSymbolInfo.SYMBOL_LEVERAGE))
        self.used_margin += X_required_margin
        self.avaliable_margin -= X_required_margin
        ...
        self.margin_manager.add_position(XSymbolInfo.SYMBOL_NAME,X_size,1 if X_size > 0 else -1,X_price,XSymbolInfo.SYMBOL_LEVERAGE)
